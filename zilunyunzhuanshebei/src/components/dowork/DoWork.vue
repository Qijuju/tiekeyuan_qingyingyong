...
export default {
  ...
  data() {
    return {
      ...
      pageNo:'1',//页数
      workflowTypeId:this._GLOBAL.workflowTypeId,//流程分类id  15代表施工日计划
      pageSize:'10',//每页条数
      workflowId:this._GLOBAL.workflowId,//流程类型id  51营业线 52临近营业线 53非营业线
      status:'1', // 0审批中 ，1审批完成
      disabled: false,
      searchTab1:false,
      searchTab2:false,
...
//打开页面是调用此方法，获取我已审批页列表数据
  mounted: function() {
    // 安全风险管理我已审批页搜索-获取流程名称值
      this.GetOnSearch();
    //安全风险管理我已审批页筛选-获取流程类型id
 ...
 //
    GetOnClickSgrjh(){
      ...
      //  获取点击审批状态事件的值
       bus.$on('van-button--normal-zt-DoWork',data=> {
         this.status = data;
         this.DoWorkflowList = []
         this.GetDoWorkflowList();
      });
