    ...
    >
      ...
      <el-table-column
        ...
        <!--<template slot-scope="scope">-->
          <!--<a :href="detailData.fj" download="fj" v-show="detailData.fj">查看</a>-->
        <!--</template>-->
        <!--<template slot-scope="scope" v-for="rowData in detailData">-->
          <!--<a v-for="item in rowData.fj_array" v-if="rowData.fj_array" :href="item.fj_url" download="fj">{{item.fj_name}}</a>-->
        <!--</template>-->
  ...
  import { Dialog } from 'vant';
    ...
    export default {
      ...
      data(){
        return{
          checked: true,
          xmqqid:this.$store.getters.staticsObj.xmqqid,// 获取通过路由传的值
      ...
      methods:{
        ...
        getData(){
          let vm = this;
          let url='http://tljjgxt.r93535.com/XmqqServlet?xmqqid='+this.xmqqid;
          // let url='http://tljjgxt.r93535.com/XmqqServlet?xmqqid='+this.gjysid;
          this.$http.get(url).then(
            res=>{
              ...
              vm.detailData.forEach(function (value,index,array) {
                //value.fj =  "27|项目信息收集表.xlsx,27|项目信息收集表.xlsx,27|项目信息收集表.xlsx"
                //value.fj =  "27|项目信息收集表.xlsx"
                switch (value.wczt){
                  case '0':
                    value.wczt = '√';
                    break;
